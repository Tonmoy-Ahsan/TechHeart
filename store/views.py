...
def search_view(request):
    ...
    products = Product.objects.filter(Q(name__icontains=k) | Q(category__category_name__icontains=k)).filter(price__gte=q).filter(price__lte=p)
...
def account_settings(request, pk):
    ...
    if request.method == 'POST':
        ...
        if information_form.is_valid():
            ...
            return render(request, 'store/account/settings.html', context)
